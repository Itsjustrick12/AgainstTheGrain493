...
public abstract class UnitAction : ScriptableObject
{
    ...
    public abstract bool IsPossible(Unit unit);
    //Holds the logic that is excuted when the action is chosen

    public abstract List<Vector3Int> GetValidTargets(Unit unit);

    public abstract bool IsAOE();

    //The unit here is the unit performing the action
    public abstract void PerformAt(Unit unit, List<Vector3Int> positions);
    //The unit here is the unit performing the action
    public abstract void PerformAt(Unit unit, Vector3Int pos);
}